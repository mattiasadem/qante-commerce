...
/** Buyer return CTA on /siparis — teslim or kargoda (local ledger). */
...
/** Buyer payment-confirm CTA on /siparis — ödeme bekliyor only (local ledger mark_paid). */
export function canConfirmPayment(status: string) {
  return status === "pending_payment";
}

/** Buyer withdraw-return CTA on /siparis — açık iade (local ledger back to fulfilled). */
export function canWithdrawReturn(status: string) {
...
export function orderProgress(status: string): { steps: string[]; active: number; cancelled?: boolean } {
  ...
  if (status === "fulfilled") return { steps: ["Ödeme", "Hazırlık", "Kargo", "Teslim"], active: 3 };
  if (status === "return_requested") return { steps: ["Ödeme", "Hazırlık", "Kargo", "Teslim", "İade"], active: 4 };
...
export function applyOrderAction(status: string, action: string): string | null {
  ...
  if (action === "withdraw_return" && status === "return_requested") return "fulfilled";
  if (action === "request_return" && (status === "fulfilled" || status === "shipped")) return "return_requested";
