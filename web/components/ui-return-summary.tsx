...
/** Shows Kolay iade · … from order note on /siparis. */
export function ReturnSummaryBanner() {
  ...
  useEffect(() => {
    ...
    void fetch(q, { cache: "no-store" }).then(async (r) => {
      ...
      const p = parseReturnFromNote(order.note);
  ...
  return (
    <p className="muted" data-cta="return-summary" style={{ marginTop: 6, marginBottom: 0 }}>
      Kolay iade · {label}
    </p>
  );
}
