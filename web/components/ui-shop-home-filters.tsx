...
export function useHomeFilters(opts: {
  ...
  const featuredProducts = useMemo(() => {
    const list = products.filter((p) => Boolean(p.featured));
    list.sort((a, b) => a.name.localeCompare(b.name, "tr"));
    return list;
  }, [products]);
  const featuredInStock = useMemo(() => {
  ...
  async function addAllFeatured() {
    if (!featuredInStock.length) return;
    setFeatBusy(true);
    try { for (const p of featuredInStock) await add(p.id, 1); }
