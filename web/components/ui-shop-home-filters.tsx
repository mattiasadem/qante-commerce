...
export function useHomeFilters(opts: {
  ...
  const compareInStock = useMemo(() => compareProducts.filter((p) => p.stock > 0), [compareProducts]);
  ...
  return {
    ...
    favOnly, recentOnly, watchOnly, favIds, recentIds, watchIds, favBusy, watchBusy, recentBusy, compareBusy, copied,
    filtered, recentProducts, favProducts, watchProducts, compareProducts, favInStock, recentInStock, watchInStock, compareInStock, watchBackCount,
    setFavFilter, setWatchFilter, setRecentFilter, pick, copyLink,
    addAllFavorites, addAllRecentInStock, addAllWatchInStock, addAllCompareInStock, clearFavorites, clearRecentViews, clearRestockWatch, clearCompare,
