...
/** Shows Gizlilik · … from order note on /siparis. */
export function GizliSummaryBanner() {
  ...
  useEffect(() => {
    ...
    void fetch(q, { cache: "no-store" }).then(async (r) => {
      ...
      const p = parseGizliFromNote(order.note);
  ...
  return (
    <p className="muted" data-cta="gizli-summary" style={{ marginTop: 6, marginBottom: 0 }}>
      Gizlilik · {label}
    </p>
  );
}
