...
type CartCtx = { cart: Cart; count: number; add: (id: string, q?: number) => Promise<void>; update: (id: string, q: number) => Promise<void>; remove: (id: string) => Promise<void>; checkout: () => Promise<DemoOrder | null> };
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => { void fetch("/api/cart", { cache: "no-store" }).then((r) => r.json()).then(setCart); }, []);
  const add = useCallback(async (productId: string, qty = 1) => { setCart(await cartCall({ action: "add", productId, qty })); }, []);
  ...
  return <CartCtx.Provider value={{ cart, count, add, update, remove, checkout }}>{children}</CartCtx.Provider>;
}
      ...
      <header className="header">
        ...
        <div className="header-actions">
          ...
          <button className="icon-btn" type="button" onClick={() => setCartOpen(true)} aria-label="Sepet">
            Sepet{count > 0 ? <span className="badge">{count}</span> : null}
          </button>
        </div>
      </header>
      <CartDrawer />
