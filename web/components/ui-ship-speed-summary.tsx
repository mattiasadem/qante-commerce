...
/** Shows Teslimat hızı · … from order note on /siparis. */
export function ShipSpeedSummaryBanner() {
  ...
  useEffect(() => {
    ...
    void fetch(q, { cache: "no-store" }).then(async (r) => {
      ...
      const p = parseShipSpeedFromNote(order.note);
  ...
  return (
    <p className="muted" data-cta="ship-speed-summary" style={{ marginTop: 6, marginBottom: 0 }}>
      Teslimat hızı · {label}
    </p>
  );
}
