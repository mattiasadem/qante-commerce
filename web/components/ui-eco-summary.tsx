...
/** Shows Çevre paketi · … from order note on /siparis. */
export function EcoSummaryBanner() {
  ...
  useEffect(() => {
    ...
    void fetch(q, { cache: "no-store" }).then(async (r) => {
      ...
      const p = parseEcoFromNote(order.note);
  ...
  return (
    <p className="muted" data-cta="eco-summary" style={{ marginTop: 6, marginBottom: 0 }}>
      Çevre paketi · {label}
    </p>
  );
}
