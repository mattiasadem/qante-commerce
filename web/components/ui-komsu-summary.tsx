...
/** Shows Komşu · … from order note on /siparis. */
export function KomsuSummaryBanner() {
  ...
  useEffect(() => {
    ...
    void fetch(q, { cache: "no-store" }).then(async (r) => {
      ...
      const p = parseKomsuFromNote(order.note);
  ...
  return (
    <p className="muted" data-cta="komsu-summary" style={{ marginTop: 6, marginBottom: 0 }}>
      Komşu · {label}
    </p>
  );
}
