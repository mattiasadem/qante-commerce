...
import { ProductCard, ProductGrid, SORTS, isLowStock, useFavorites, useRecentViews, type SortId } from "@/components/ui-shop-core";
...
export function HomeView({
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [products, sort, inStockOnly, onSaleOnly, lowStockOnly, favOnly, favIds, recentOnly, recentIds]);
  ...
  const showFeatured =
    !query && !category && featured.length && sort === "default" && !inStockOnly && !onSaleOnly && !lowStockOnly && !favOnly && !recentOnly;
  const showRecentRail =
    !query && !category && recentProducts.length > 0 && sort === "default" && !inStockOnly && !onSaleOnly && !lowStockOnly && !favOnly && !recentOnly;
  return (
