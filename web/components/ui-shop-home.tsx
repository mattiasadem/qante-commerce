...
export function HomeView({
  ...
  const h = useHomeFilters({
    products, query, category, initialFav, initialSort, initialStock, initialSale, initialLow, initialRecent, initialWatch,
  });
  const showFeatured =
    !query && !category && featured.length && h.sort === "default" && !h.inStockOnly && !h.onSaleOnly && !h.lowStockOnly && !h.favOnly && !h.recentOnly && !h.watchOnly;
  ...
  const showCompareRail =
    !query && !category && h.compareProducts.length > 0 && h.sort === "default" && !h.inStockOnly && !h.onSaleOnly && !h.lowStockOnly && !h.favOnly && !h.recentOnly && !h.watchOnly;
  ...
  return (
    <div className="grid-wrap">
      ...
      <div className="section-label">
        {query ? `${h.filtered.length} sonuç · ${query}` : h.favOnly ? `Favoriler · ${h.filtered.length}` : h.recentOnly ? `Son bakılanlar · ${h.filtered.length}` : h.watchOnly ? `Beklediklerim · ${h.filtered.length}` : category ? category : "Katalog"}
