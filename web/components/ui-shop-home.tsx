  ...
  return (
    <div className="grid-wrap">
      ...
      {showLowRail ? (
        <>
          <div className="section-label" style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
            <span>Az stok · {h.lowStockProducts.length}</span>
            {h.lowStockReady.length ? (
              ...
              >
                {h.lowBusy ? "ekleniyor…" : `Tümünü sepete · ${h.lowStockReady.length}`}
              </button>
            ) : null}
          ...
          <div className="featured" data-rail="low-stock">{h.lowStockProducts.slice(0, 4).map((p) => (<ProductCard key={p.id} product={p} />))}</div>
        </>
      ) : null}
      {showCompareRail ? (
        <>
              ...
              : h.favOnly
                ...
                : h.recentOnly
                  ...
                  : h.onSaleOnly
                    ...
                    : h.lowStockOnly
                      ? "Şu an acil stok yok; filtreyi kapat veya stok sıralamasını dene."
                      : undefined
        }
      />
      <ShopFooter />
