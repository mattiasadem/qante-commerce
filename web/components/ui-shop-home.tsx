...
import { ProductCard, ProductGrid, SORTS, isLowStock, useFavorites, useRecentViews, useRestockWatch, type SortId } from "@/components/ui-shop-core";
...
export function HomeView({
  ...
  const watchInStock = useMemo(
    () => (watchOnly ? filtered.filter((p) => p.stock > 0) : []),
    [filtered, watchOnly],
  );
  ...
  const showFeatured =
    !query && !category && featured.length && sort === "default" && !inStockOnly && !onSaleOnly && !lowStockOnly && !favOnly && !recentOnly && !watchOnly;
  const showRecentRail =
    !query && !category && recentProducts.length > 0 && sort === "default" && !inStockOnly && !onSaleOnly && !lowStockOnly && !favOnly && !recentOnly && !watchOnly;
  return (
    <div className="grid-wrap">
      ...
      <div className="chips scroll" data-chips="sort" role="tablist" aria-label="Sırala" style={{ marginTop: 8 }}>
        ...
        <button
          className={`chip ${lowStockOnly ? "on" : ""}`}
          type="button"
        ...
        <button
          ...
          data-filter="restock-watch"
          onClick={() => setWatchFilter(!watchOnly)}
        >
          Beklediklerim{watchIds.length ? ` · ${watchIds.length}` : ""}
          {watchBackCount ? ` · ${watchBackCount} geldi` : ""}
        </button>
        {watchOnly && watchInStock.length ? (
          <button
            className="chip on"
            type="button"
            data-cta="watch-add-all"
            disabled={watchBusy}
            onClick={() => void addAllWatchInStock()}
          >
            {watchBusy ? "ekleniyor…" : `Gelenleri sepete ekle · ${watchInStock.length}`}
          </button>
        ) : null}
        {watchOnly && watchIds.length ? (
          <button
            className="chip"
            type="button"
            data-cta="clear-restock-watch"
            onClick={() => {
              clearRestockWatch();
              setWatchOnly(false);
            }}
