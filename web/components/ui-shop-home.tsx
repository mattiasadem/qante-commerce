  ...
  return (
    <div className="grid-wrap"><div className="hero-row"><h1>{greeting}</h1><span className="date-line">{dateLabel}</span></div><p className="lede">Keten,yün ve ev. Filtreler URL'de; Kopyala ile paylaş.</p><div className="chips scroll" data-chips="category">
      ...
      <div className="section-label">
        ...
        emptyHint={
          h.compareOnly
            ? "Kartta Karşılaştır ile ekle; tabloda yan yana bak."
            : h.watchOnly
              ? "Tükenen üründe Gelince haber ver; stok gelince burada. Takipte ile çıkar."
              : h.favOnly
                ? "Kalple favoriye ekle."
                : h.recentOnly
                  ? "Ürün sayfalarına bakınca birikir."
                  : h.featuredOnly
                    ? "Öne çıkan yok; filtreyi kapat veya kategori değiştir."
                  : h.outOfStockOnly
                    ? "Tükenen yok; Gelince haber ver ile takip et."
                  : h.onSaleOnly
                    ? "İndirimli yok; filtreyi kapat veya kategori değiştir."
                    : h.lowStockOnly
                      ? "Az stok yok; filtreyi kapat veya stok sırala."
                      : undefined
        }
      /><ShopFooter /></div>
  );
