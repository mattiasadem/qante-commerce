...
/** Shows Kapıcıya bırak · … from order note on /siparis. */
export function DoormanSummaryBanner() {
  ...
  useEffect(() => {
    ...
    void fetch(q, { cache: "no-store" }).then(async (r) => {
      ...
      const p = parseDoormanFromNote(order.note);
  ...
  return (
    <p className="muted" data-cta="doorman-summary" style={{ marginTop: 6, marginBottom: 0 }}>
      {label}
    </p>
  );
}
