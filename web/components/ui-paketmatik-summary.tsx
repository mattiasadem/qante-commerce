...
/** Shows Paketmatik · … from order note on /siparis. */
export function PaketmatikSummaryBanner() {
  ...
  useEffect(() => {
    ...
    void fetch(q, { cache: "no-store" }).then(async (r) => {
      ...
      const p = parsePaketmatikFromNote(order.note);
  ...
  return (
    <p className="muted" data-cta="paketmatik-summary" style={{ marginTop: 6, marginBottom: 0 }}>
      Paketmatik · {label}
    </p>
  );
}
